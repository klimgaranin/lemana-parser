...
Для режима `--articles` парсер сначала запрашивает данные стандартным API-запросом, а если часть артикулов не вернулась, повторяет только недостающие товары без фасетов текущей категории и без `filterByEligibility`. Большие списки артикулов идут отдельными осторожными батчами: при `403/429/5xx` API-запрос повторяется после cooldown, а проблемный batch автоматически делится на меньшие части. Если API всё равно не отдаёт товар, строка остаётся в Excel со статусом `api_data_missing` или `api_request_failed`.
...
- `--api-article-batch-size` — размер API-батча для произвольного списка артикулов.
- `--api-request-sleep` — пауза между API-батчами по артикулам.
- `--api-max-retries` и `--api-antibot-cooldown` — повторы и cooldown API после `403/429/5xx`.
...
Это резко уменьшает количество запросов к сайту: вместо отдельной HTML-загрузки каждой карточки используются пачки до 60 товаров. Для выгрузки произвольных списков через `--articles-file` используется более бережный размер batch, потому что такие списки могут содержать товары из разных категорий и сильнее триггерить антибот. `api-fallback` является основным режимом: при проблеме API каталога парсер автоматически вернётся к HTML-сценарию.

### API debug-логи

Для разбора `403/429/5xx` парсер пишет структурные API debug-логи в `logs/api_debug/*.jsonl`. В них видно метод, URL, query-параметры, payload, размер batch, статус ответа, headers и короткий body ответа. Секреты (`Cookie`, `x-api-key`, `Authorization`) маскируются. Старые debug-логи удаляются автоматически, по умолчанию через 7 дней.

Основные настройки:

- `LEMANA_API_DEBUG_LOG_ENABLED=true`
- `LEMANA_API_DEBUG_LOG_DIR="logs/api_debug"`
- `LEMANA_API_DEBUG_LOG_RETENTION_DAYS=7`
- `LEMANA_API_DEBUG_LOG_BODY_LIMIT=8000`
- `LEMANA_API_DEBUG_LOG_SUCCESS_BODY=false`
